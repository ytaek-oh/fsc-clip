...
def multi_head_attention_forward(
    ...
    if not use_separate_proj_weight:
        if torch.equal(query, key) and torch.equal(key, value):
            ...
            qkv += (
                linear(linear(query, in_proj_weight_A), in_proj_weight_B) * in_proj_weight_scaling
            )
            ...
            q += (
                linear(linear(query, in_proj_weight_A), in_proj_weight_B[_start:_end, :]) *
    ...
    if attn_mask is not None:
        assert (
            attn_mask.dtype == torch.float32 or attn_mask.dtype == torch.float64
            or attn_mask.dtype == torch.float16 or attn_mask.dtype == torch.uint8
    ...
    attn_output += (
        linear(linear(attn_output_, out_proj_weight_A), out_proj_weight_B) * out_proj_weight_scaling
    )
