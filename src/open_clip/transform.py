...
from torchvision.transforms import (
    CenterCrop, ColorJitter, Compose, Grayscale, InterpolationMode, Normalize, RandomResizedCrop,
    Resize, ToTensor,
)
...
def merge_preprocess_dict(
    ...
    if overlay:
        overlay_clean = {
            k: v
            for k, v in overlay.items() if k in _PREPROCESS_KEYS and v is not None
        }
...
class AugmentationCfg:
    ...
    color_jitter: Optional[Union[float, Tuple[float, float, float], Tuple[float, float, float,
                                                                          float]]] = None
...
class ResizeKeepRatio:
    ...
    def __call__(self, img):
        ...
        size = self.get_params(
            img, self.size, self.longest, self.random_scale_prob, self.random_scale_range,
